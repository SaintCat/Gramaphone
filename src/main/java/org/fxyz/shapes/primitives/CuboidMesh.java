...
//            smoothingGroups=SmoothingGroups.calcSmoothGroups(new TriangleMesh(), newFaces, newFaceNormals, normals);
    ...
    public void updateFigure() {
        switch (currentState) {
            case 1:
                double radius = Math.sqrt(2) * getWidth() / 2;
                for (Point3D p : listVertices) {
                    if (p.z != 0 && p.y != 0 && Math.abs(p.x) == getWidth() / 2) {
                        float y = (float) Math.sqrt((radius * radius) / ((float) (Math.pow(p.x / p.z, 2) + 1)));
                        float x = (float) ((float) p.x / (float) p.z) * (float) y;
                        if (p.x > 0) {
                            p.x = Math.abs(x);
                        } else {
                            p.x = -Math.abs(x);

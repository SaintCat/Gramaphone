...
public class SegmentedTorusMesh extends TexturedMesh {
    ...
    private Map<MutableFloat, List<Point3D>> values;
    ...
    private TriangleMesh createTorus(int subDivX, int subDivY, int crop, float meanRadius,
        ...
        for (Point3D p : listVertices) {
            ...
            if (!values.containsKey(new MutableFloat(angle))) {
                values.put(new MutableFloat(angle), new ArrayList<>());
                values.get(new MutableFloat(angle)).add(p);
            } else {
                values.get(new MutableFloat(angle)).add(p);
            }
        }
        for (MutableFloat val : values.keySet()) {
    ...
    public void updateFigure() {
        switch (currentState) {
            case 1:
                float i = 0;
//                for (MutableFloat key : values.keySet()) {
//                    if (i == 0) {
//                        i++;
//                        continue;
//                    }
//                    rotateByKey(val, 0.01 + i/100);
//                MutableFloat val = (MutableFloat) values.keySet().toArray()[0];
//                    increaseRadius(val, 0.004f);
                MutableFloat key = (MutableFloat) values.keySet().toArray()[7];
...
//                key = (MutableFloat) values.keySet().toArray()[5];
...
//                }
     ...
     private List<Point3D> findVectors(List<Point3D> points, Point3D point3D) {
    ...
    private void increaseRadius(MutableFloat key, float val) {
        List<Point3D> points = values.get(key);
    ...
    private void rotateByKey(MutableFloat key, double angle) {
        List<Point3D> points3 = values.get(key);
        for (Point3D p : points3) {
            MatrixOperations.rotPoint(p, angle);

        }
//        Float newKey = key + 0.01f;
        key.add(angle);
    }
